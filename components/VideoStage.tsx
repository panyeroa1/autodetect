...
export const VideoStage: React.FC<VideoStageProps> = ({ 
  ...
  return (
    <div className="relative w-full h-full overflow-hidden flex items-center justify-center bg-black/20 backdrop-blur-[2px]">
      ...
      {/* --- LAYER 2: SUBTITLES (Glass Overlay) --- */}
      {showCaptions && liveCaption && (
          <div className="absolute bottom-32 left-0 right-0 z-30 flex justify-center px-6 pointer-events-none">
             <div className="max-w-4xl w-full text-center animate-in slide-in-from-bottom-6 fade-in duration-500">
                <div className="bg-black/60 backdrop-blur-lg rounded-3xl p-6 inline-block shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/5">
                    <p className="text-xl md:text-3xl text-white font-light leading-relaxed tracking-wide drop-shadow-md">
                       "{liveCaption.translatedText || liveCaption.originalText}"
                    </p>
                    {liveCaption.translatedText && (
                        <p className="text-sm text-zinc-400 mt-2 font-light italic opacity-80 tracking-wide">
