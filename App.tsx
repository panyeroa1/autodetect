...
export default function App() {
  ...
  const saveUserToStorage = (user: User) => {
    try {
      ...
      try {
        // If avatar is huge (base64), stripping it might save the day
        const slimUser = { ...user, avatar: '' }; 
        // We first remove the item to clear space, then set the slim version
  ...
  useEffect(() => {
    ...
    const channel = supabase
        ...
        // Listen for interim transcript broadcasts for real-time captions
        .on('broadcast', { event: 'interim_transcript' }, (payload) => {
            const { userId, text } = payload.payload;
            if (userId !== currentUser.id) {
                showCaptionHandler(userId, text);
                setSpeakingUserId(userId);
                // Note: The final DB insert will trigger the completion logic which clears the speaking indicator.
            }
        })
        .subscribe();
  ...
  useEffect(() => {
    if (transcript && activeGroupId && currentUser && messagesSubscription.current) {
        messagesSubscription.current.send({
            type: 'broadcast',
            event: 'interim_transcript',
            payload: { userId: currentUser.id, text: transcript }
        });
        // Also show locally immediately
        showCaptionHandler(currentUser.id, transcript);
        setSpeakingUserId(currentUser.id);
    }
  }, [transcript, activeGroupId, currentUser]);
